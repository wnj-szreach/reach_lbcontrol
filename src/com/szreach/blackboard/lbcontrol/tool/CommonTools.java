		...
		for (Field field : fields) {
			try {
				if (field.getType() != boolean.class) {
					getMethod = obj.getClass().getMethod("get" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1), null);
				} else {
					getMethod = obj.getClass().getMethod("is" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1), null);
				}
				Object retValue = getMethod.invoke(obj, null);
				if (retValue != null) {
					key = field.getName();
					if (field.getType() == Date.class) {
						value = CommonTools.dateToString((Date) retValue, CommonTools.DF_FULL_PATTERN);
					} else if (field.getType() == List.class || field.getType() == ArrayList.class) {
						value = listToJson((List) retValue);
					} else {
